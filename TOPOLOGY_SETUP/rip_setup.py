...
def rip():
    # A function to get networks enabled for rip.

    print("\033[1;93m")
    print("This function assisting you to define the networks enabled for rip.")
    ...
    while TRACK_INPUT == 0:
        USER_INPUT = input("\033[1;93m Enter a network enabled for rip (eg '11.0.0.0/8', RESPECT THE FORMAT), then press 'ENTER' \n: \033[0m")
        if USER_INPUT == "EXIT" and RIP_LIST != []:
            TRACK_INPUT = 1
            print("\033[1;93m Exit the rip loop. \033[0m")
        elif USER_INPUT == "EXIT" and RIP_LIST == []:
            print("\033[1;93m You have not provided any rip network. \033[0m")
        else:
            RIP_LIST.append(USER_INPUT)
...
def setup(INPUT_DICT_PER_HOST):
    ...
    for element_1 in RIP_LIST:
        for element_2 in INTERFACE_ADDRESS_PAIRS_LIST:
            if CompareAddresses(element_1, element_2[1]).check_for_subnet():
                Cn = "network " + element_1.split("/")[0]
                RIP_COMMANDS.append(Cn)

    # Remove duplicates.
    EMPTY_LIST = []
    for element in RIP_COMMANDS:
        if element not in EMPTY_LIST:
            EMPTY_LIST.append(element)
    RIP_COMMANDS = EMPTY_LIST

    # Check if there is any network configuration.
    if len(RIP_COMMANDS) == 3:
        RIP_COMMANDS = []
    else:
        pass
