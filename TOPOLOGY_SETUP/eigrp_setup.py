...
def eigrp():
    # A function to get AS and networks enabled for eigrp.

    print("\033[1;93m")
    print("This function assisting you to define the AS and networks enabled for eigrp.")
    ...
    while TRACK_INPUT == 0:
        USER_INPUT = input("\033[1;93m Enter the AS number for the eigrp (eg '101', RESPECT THE FORMAT), then press 'ENTER' \n: \033[0m")
        if USER_INPUT == "EXIT" and X == []:
            print("\033[1;93m You have not provided any eigrp AS number. \033[0m")
        else:
            X.append(USER_INPUT)
            TRACK_INPUT = 1
    EIGRP_LIST.append(X)
    ...
    while TRACK_INPUT == 0:
        USER_INPUT = input("\033[1;93m Enter a network enabled for eigrp (eg '11.0.0.0/8', RESPECT THE FORMAT), then press 'ENTER' \n: \033[0m")
        if USER_INPUT == "EXIT" and Y != []:
            TRACK_INPUT = 1
            print("\033[1;93m Exit the eigrp loop. \033[0m")
        elif USER_INPUT == "EXIT" and Y == []:
            print("\033[1;93m You have not provided any eigrp network. \033[0m")
        else:
            Y.append(USER_INPUT)
    EIGRP_LIST.append(Y)
...
def setup(INPUT_DICT_PER_HOST):
    ...
    # EIGRP commands.
    EIGRP_COMMANDS = []
    C1 = "router eigrp " + EIGRP_LIST[0][0]
    C2 = "no auto-summary"
    EIGRP_COMMANDS.append(C1)
    EIGRP_COMMANDS.append(C2)
    for element_1 in EIGRP_LIST[1:]:
        for element_2 in element_1:
            for element_3 in INTERFACE_ADDRESS_PAIRS_LIST:
                if CompareAddresses(element_2, element_3[1]).check_for_subnet():
                    Cn = "network " + element_2.split("/")[0] + " " + str(Address(element_2).extrack_mask_wildcard_form())
    ...
    # Remove duplicates.
    EMPTY_LIST = []
    for element in EIGRP_COMMANDS:
        if element not in EMPTY_LIST:
            EMPTY_LIST.append(element)
    EIGRP_COMMANDS = EMPTY_LIST

    # Check if there is any network configuration.
    if len(EIGRP_COMMANDS) == 2:
        EIGRP_COMMANDS = []
    else:
        pass
